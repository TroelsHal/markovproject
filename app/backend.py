...
def GetLyrics(textfile):
    ...
    # Transform to lowercase, removes punctuation, and splits to list of strings
        ...
        if not (word in lyricDict):
            ...
            for j in range(0, len(lyricDict[word])):
                if lyricDict[word][j][0] == nextWord:
                    index = j
            #is this an existing word, but a new nextWord
            if index == -1:
                lyricDict[word].append([nextWord, 1])
            #is this an existing word, and an existing newWord 
            else:
...
def MakeSong(lyricDict):
    ...
    # the title is either the beginning of the 1. verse (1/3 chance)
    #  or the beginning of the chorus (2/3 chance)
    ...
    if verse3:
        song = song + f"<p>{verse2}<p>" + f"<p><i>{chorus}</i><p>" + \
            f"<p>{verse3}<p>" + f"<p><i>{chorus}</i><p>"
    else:
        song = song + f"<p><i>{chorus}</i><p>" + f"<p>{verse2}<p>" + \
            f"<p><i>{chorus}</i><p>" + f"<p><i>{chorus}</i><p>"
    return song
...
def MakeText(lyricDict, lines, linelength, part):
    ...
    for i in range(0, lines):
        ...
        for j in range(0, linelength):
            ...
            line += "<br>" if (j == (linelength / 2) -
                               1 and part == "verse") else " "
